...
public class VerticalRollingTextView extends View {
    ...
    private void parseAttrs(Context context, AttributeSet attrs) {
        ...
        mPaint.setColor(typedArray.getColor(R.styleable.VerticalRollingTextView_android_textColor, Color.WHITE));
    ...
    protected void onDraw(Canvas canvas) {
        // 绘制文本
        if (mDataSetAdapter == null || mDataSetAdapter.isEmpty()||mDataSetAdapter.getItemCount() == 0) {
            return;
        }
        ...
        if (mCurrentIndex ==0&&mNextIndex==0){
            canvas.drawText(text1, mWidth / 2 - (int) (measureText(text1) / 2), mCurrentOffsetY, mPaint);
        }else {
            canvas.drawText(text1, mWidth / 2 - (int) (measureText(text1) / 2), mCurrentOffsetY, mPaint);
            canvas.drawText(text2, mWidth / 2 - (int) (measureText(text2) / 2), mCurrentOffsetY + mOffset + mTextTopToAscentOffset, mPaint);
        }
    }
    ...
    private float measureText(String text1) {
        return mPaint.measureText(text1);
    }
